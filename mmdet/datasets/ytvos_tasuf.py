...
class YTVOSDatasetTASUF(CustomDataset):
    ...
    def sample_ref_seq(self, idx):
        ...
        if frame_id < seq_len:
            valid_samples = self.sample_ref_range(frame_id, vid_len, seq_len, backward=True)
        elif frame_id + seq_len > vid_len:
            valid_samples = self.sample_ref_range(0, frame_id, seq_len, backward=False)
        else:
    ...
    def prepare_train_img(self, idx):
        ...
        gt_pids_seq.append([id_set.index(i)+1 if i in id_set else 0 for i in gt_ids])
