...
class TwoStageDetectorSUF(BaseDetector, RPNTestMixin,
    ...
    def forward_train(self,
        ...
        ref_x_list = list(map(self.extract_feat, ref_img_list))
        ...
        if self.with_bbox:
            ...
            ref_rois_list = list(map(bbox2roi, ref_bboxes_list))
            # ref_rois = bbox2roi(ref_bboxes)

            ref_bbox_img_n_list = [ [ x.size(0) for x in ref_bboxes ] for ref_bboxes in ref_bboxes_list ]
            ...
            ref_bbox_feats_list = []
            for ref_x, ref_rois in zip(ref_x_list, ref_rois_list):
                ref_bbox_feats_list.append(self.bbox_roi_extractor(ref_x[:self.bbox_roi_extractor.num_inputs], ref_rois))
            ...
            match_score = self.track_head(bbox_feats, ref_bbox_feats_list, 
                                        bbox_img_n, ref_bbox_img_n_list,
                                        gt_pids_list)
    ...
    def simple_test_bboxes(self,
        ...
        if is_first or (not is_first and self.prev_bboxes is None):
            ...
            self.prev_roi_feats = self.track_head.init_hidden_states(det_roi_feats)
            ...
            for idx, match_id in enumerate(match_ids):
                if match_id == 0:
                    ...
                    hidden_states = self.track_head.init_hidden_states(det_roi_feats[idx][None].unsqueeze(0)) # (1, h&c=2, num_layers=2, batch_size=1, hidden_size=1024)
                    self.prev_roi_feats = torch.cat((self.prev_roi_feats, hidden_states), dim=0) # (#objects + 1, h&c=2, num_layers=2, hidden_size=1024)
                    ...
                    if match_score > best_match_scores[obj_id]:
                        ...
                        # udpate feature
                        self.prev_roi_feats[obj_id] = \
                            self.track_head.update_hidden_state(det_roi_feats[idx][None], self.prev_roi_feats[obj_id])
